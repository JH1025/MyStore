...
public class SearchingParse {
	
String url ="http://183.106.250.84/searchProduct.php";
	
	public SearchingParse(){
		
	}
	//추후에...
	public void findUserAndParseInfo(User user,String target) {
		// TODO Auto-generated method stub
		
		try{
			String Json=sendData(target);
			if(Json == null){
				user = null;
				return;	
			}
	...
	public String sendData(String target) throws ClientProtocolException ,IOException{
		HttpPost request = makeHttpPost(target,url);
	...
	private HttpPost makeHttpPost( String $target,String $url){
		HttpPost request = new HttpPost($url);
		Vector<NameValuePair> nameValue = new Vector<NameValuePair>();
		nameValue.add(new BasicNameValuePair("target",$target));
